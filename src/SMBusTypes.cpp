...
const SMBusProtocol SMBusProtocols[ NUM_SMBUS_PROTOCOLS ] = {
    ...
    { "Block process call",
      { SMBP_Start, SMBP_AddrWrite, SMBP_Command, SMBP_DataWord, SMBP_Start, SMBP_AddrRead, SMBP_DataWord, SMBP_Stop } },
    ...
    { "Block read",
      { SMBP_Start, SMBP_AddrWrite, SMBP_Command, SMBP_ByteCount, SMBP_DataBlock, SMBP_Start, SMBP_AddrRead, SMBP_ByteCount, SMBP_DataBlock,
        SMBP_Stop } },
...
const SMBusProtocol PMBusProtocols[ NUM_PMBUS_PROTOCOLS ] = {
    ...
    { "Ext command read word",
      { SMBP_Start, SMBP_AddrWrite, SMBP_CommandExt, SMBP_Start, SMBP_AddrRead, SMBP_DataByte, SMBP_DataByte, SMBP_Stop } },
...
void SMBusPacket::CreateFrames( SMBusAnalyzerResults* pResults, const SMBusProtocol* pProt, bool has_pec, SMBusDecodeLevel dec_level ) const
{
    ...
    if( dec_level == DL_PMBus && pProt != SMBus_QuickCommand )
    {
        ...
        if( pProt == SMBus_SendByte && ( cmd.id == CLEAR_FAULTS || cmd.id == STORE_DEFAULT_ALL || cmd.id == RESTORE_DEFAULT_ALL ||
                                         cmd.id == STORE_USER_ALL || cmd.id == RESTORE_USER_ALL ) )
        {
            pResults->AddFrame( chunks.front()[ 0 ].ToAddrFrame( pProt ) );
        ...
        else if( pProt == SMBus_WriteByte && ( cmd.direction == Write || cmd.direction == ReadWrite ) &&
        ...
        else if( pProt == SMBus_ReadByte && ( cmd.direction == Read || cmd.direction == ReadWrite ) &&
                 cmd.databyte_param_type != FT_Undefined )
        {
            pResults->AddFrame( chunks.front()[ 0 ].ToAddrFrame( pProt ) );
            pResults->AddFrame( chunks.front()[ 1 ].ToPMBusCommandFrame() );

            pResults->AddFrame( chunks[ 1 ][ 0 ].ToAddrFrame( pProt ) );
            pResults->AddFrame( chunks[ 1 ][ 1 ].ToDataByte( cmd.databyte_param_type ) );
            if( chunks[ 1 ].size() > 2 )
                pResults->AddFrame( chunks[ 1 ][ 2 ].ToPECFrame( CalcPEC( has_pec ) ) );
        }
        else if( pProt == SMBus_BlockProcessCall && chunks.front()[ 1 ].value == QUERY && chunks.front().size() == 4 &&
    ...
    else if( dec_level == DL_SmartBattery && pProt != SMBus_QuickCommand )
    {
        ...
        if( pProt == SMBus_WriteWord && ( cmd.direction == Write || cmd.direction == ReadWrite ) &&
            cmd.databyte_param_type != FT_Undefined )
        {
            pResults->AddFrame( chunks.front()[ 0 ].ToAddrFrame( pProt ) );
        ...
        else if( pProt == SMBus_ReadWord && ( cmd.direction == Read || cmd.direction == ReadWrite ) &&
                 cmd.databyte_param_type != FT_Undefined )
        {
            pResults->AddFrame( chunks.front()[ 0 ].ToAddrFrame( pProt ) );
            pResults->AddFrame( chunks.front()[ 1 ].ToSmartBatteryCommandFrame() );
...
void SMBusPacket::CreateFramesForRawData( SMBusAnalyzerResults* pResults, bool has_pec ) const
{
    for( std::vector<std::vector<SMBusByte>>::const_iterator ci( chunks.begin() ); ci != chunks.end(); ++ci )
    {
        ...
        pResults->AddFrame( chunk.front().ToAddrFrame( 0 ) ); // addr
        pec = SMBusCRCLookup[ pec ^ chunk.front().value ];
...
U8 SMBusPacket::CalcPEC( bool has_pec ) const
{
    ...
    for( std::vector<std::vector<SMBusByte>>::const_iterator ci( chunks.begin() ); ci != chunks.end(); ++ci )
    {
        for( std::vector<SMBusByte>::const_iterator bi( ci->begin() ); bi != ci->end(); ++bi )
        {
...
bool SMBusPacket::MatchesProtocol( const SMBusProtocol* pProt, bool has_pec ) const
{
    ...
    while( pProtElems[ prot_elem_ndx ] != SMBP_Stop )
    {
        ...
        if( pProtElems[ prot_elem_ndx ] != SMBP_Start && pProtElems[ prot_elem_ndx ] != SMBP_Stop && chunk.size() <= bcnt )
            return false;
